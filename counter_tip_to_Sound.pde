 ...
 */
  ...
  moog4 = new MoogFilter( 50*50, 0.5 );
...
//**************** IF MOTOR TURN IN Clock Way and number of revolution is >0
    // revoltion pair
       ...
       phazi[i]= constrain (map( net.phase[i], 0, 2*PI, 0, 20000), freqMin, freqMax);  
       println ("nombre pair");
     }
     // revoltion impair
        if (net.phase[i] > 0 && j[i]==1 )  {   
        phazi[i]= constrain (map( net.phase[i], 0, 2*PI, 20000, 0), freqMin, freqMax);
    ...
    //*******************************$ // IF MOTOR TURN IN Clock Way   and number of revolution is <0 
        ...
        speedi[i]= map ((net.velocity[i]*100), -300, 300, 0, 1);
        
        
        rez= constrain( map( orderParameter, 0, 1, -1, 1), 0, 1 );   
        resonancei[i]= speedi[i]/rez;
    ...
    */
       ...
       moog4.frequency.setLastValue( phazi[4]  );
  ...
  */
       moog4.resonance.setLastValue( resonancei[4]); //rez
